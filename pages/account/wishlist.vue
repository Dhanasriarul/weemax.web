          ...
          >
            <div class="py-6 sm:flex">
              ...
              <div class="mt-6 space-y-4 sm:ml-6 sm:mt-0 sm:w-40 sm:flex-none">
                <!-- <button
                  v-if="product.isAddedToBasket"
                  @click="removeBasket(product.productId)"
                ...
                <button
                  v-else
                  @click="addBasket(product.productId)"
                  class="flex w-full items-center justify-center rounded-md border border-transparent bg-black px-2.5 hover:scale-110 transition py-2 text-sm font-medium text-white shadow-sm focus:outline-none focus:ring-2 focus:ring-black focus:ring-offset-2 sm:w-full sm:flex-grow-0"
                >
                  Add to basket
                </button> -->
                <button
                  ...
                  class="flex w-full items-center justify-center rounded-md border border-gray-300 bg-white px-2.5 py-2 text-sm hover:scale-110 transition font-medium text-gray-700 shadow-sm hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-black focus:ring-offset-2 sm:w-full sm:flex-grow-0"
                >
                  Remove
                </button>
              </div>
...
export default {
  ...
  methods: {
    ...
    addBasket(id) {
      this.$http
        .$post(`/product/basket?productId=${id}`, {
          headers: {
            Authorization: `Bearer ${window.localStorage.getItem(
              "weemaxAccessToken"
            )}`,
    ...
    removeBasket(id) {
      this.$http
        .$delete(`/product/basket?productId=${id}`, {
          headers: {
            Authorization: `Bearer ${window.localStorage.getItem(
              "weemaxAccessToken"
            )}`,
    ...
    removeWishlist(id) {
      this.$http
        .$delete(`/product/wishlist?productId=${id}`, {
          headers: {
            Authorization: `Bearer ${window.localStorage.getItem(
              "weemaxAccessToken"
            )}`,
